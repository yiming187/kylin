...
@Slf4j
public class SortProjectTransposeRuleTest extends CalciteRuleTestBase {
    ...
    protected DiffRepository getDiffRepo() {
        return DiffRepository.lookup(SortProjectTransposeRuleTest.class);
    }

    @Test
    public void testSortProjectTranspose() throws IOException {
        final List<RelOptRule> rules = new ArrayList<>(HepUtils.AggPushDownRules);
        List<Pair<String, String>> queries = readALLSQLs(KylinConfig.getInstanceFromEnv(), defaultProject,
                "query/sql_sort_project");
