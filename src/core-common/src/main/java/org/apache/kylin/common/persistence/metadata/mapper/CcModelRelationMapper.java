...
import static org.apache.kylin.common.persistence.metadata.mapper.CcModelRelationDynamicSqlSupport.sqlTable;
...
public interface CcModelRelationMapper extends BasicMapper<CcModelRelationRawResource> {
    @Override
    default BasicColumn[] getSelectList() {
        return getSelectListWithAdditions(sqlTable.ccUuid, sqlTable.modelUuid);
    }
    ...
    @Override
    default UpdateDSL<UpdateModel> updateAllColumns(CcModelRelationRawResource record, UpdateDSL<UpdateModel> dsl) {
        dsl = BasicMapper.super.updateAllColumns(record, dsl);
        return dsl.set(sqlTable.ccUuid).equalTo(record::getCcUuid).set(sqlTable.modelUuid)
    ...
    @Override
    @SelectProvider(type = SqlProviderAdapter.class, method = "select")
    @Results(id = "CcModelRelationResult", value = {
            ...
            @Result(column = "cc_uuid", property = "ccUuid", jdbcType = JdbcType.CHAR),
    ...
    List<CcModelRelationRawResource> selectMany(SelectStatementProvider selectStatement);

    @Override
    @SelectProvider(type = SqlWithRecordLockProviderAdapter.class, method = "select")
    @ResultMap("CcModelRelationResult")
    List<CcModelRelationRawResource> selectManyWithRecordLock(SelectStatementProvider selectStatement);
}
