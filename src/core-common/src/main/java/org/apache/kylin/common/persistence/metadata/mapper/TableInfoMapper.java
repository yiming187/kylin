...
import static org.apache.kylin.common.persistence.metadata.mapper.TableInfoDynamicSqlSupport.sqlTable;
...
public interface TableInfoMapper extends BasicMapper<TableInfoRawResource> {
    @Override
    ...
    Optional<TableInfoRawResource> selectOne(SelectStatementProvider selectStatement);

    @Override
    @SelectProvider(type = SqlProviderAdapter.class, method = "select")
    @Results(id = "TableInfoResult", value = {
    ...
    List<TableInfoRawResource> selectMany(SelectStatementProvider selectStatement);

    @Override
    @SelectProvider(type = SqlWithRecordLockProviderAdapter.class, method = "select")
    @ResultMap("TableInfoResult")
    List<TableInfoRawResource> selectManyWithRecordLock(SelectStatementProvider selectStatement);
}
