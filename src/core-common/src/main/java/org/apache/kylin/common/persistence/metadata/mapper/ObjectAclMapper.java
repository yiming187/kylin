...
import static org.apache.kylin.common.persistence.metadata.mapper.ObjectAclDynamicSqlSupport.sqlTable;
...
public interface ObjectAclMapper extends BasicMapper<ObjectAclRawResource> {
    @Override
    ...
    @Override
    @SelectProvider(type = SqlProviderAdapter.class, method = "select")
    @Results(id = "ObjectAclResult", value = {
    ...
    List<ObjectAclRawResource> selectMany(SelectStatementProvider selectStatement);

    @Override
    @SelectProvider(type = SqlWithRecordLockProviderAdapter.class, method = "select")
    @ResultMap("ObjectAclResult")
    List<ObjectAclRawResource> selectManyWithRecordLock(SelectStatementProvider selectStatement);
}
