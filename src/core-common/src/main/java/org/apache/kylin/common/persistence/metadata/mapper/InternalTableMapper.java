...
import static org.apache.kylin.common.persistence.metadata.mapper.InternalTableDynamicSqlSupport.internalTable;
...
public interface InternalTableMapper extends BasicMapper<InternalTableRawResource> {
    @Override
    ...
    @ResultMap("InternalTableResult")
    Optional<InternalTableRawResource> selectOne(SelectStatementProvider selectStatement);

    @Override
    @SelectProvider(type = SqlProviderAdapter.class, method = "select")
    @Results(id = "InternalTableResult", value = {
    ...
    List<InternalTableRawResource> selectMany(SelectStatementProvider selectStatement);

    @Override
    @SelectProvider(type = SqlWithRecordLockProviderAdapter.class, method = "select")
    @ResultMap("InternalTableResult")
    List<InternalTableRawResource> selectManyWithRecordLock(SelectStatementProvider selectStatement);
}
