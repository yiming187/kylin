...
import static org.apache.kylin.common.persistence.metadata.mapper.SystemDynamicSqlSupport.sqlTable;
...
public interface SystemMapper extends BasicMapper<SystemRawResource> {
    @Override
    ...
    @Override
    default UpdateDSL<UpdateModel> updateAllColumns(SystemRawResource record, UpdateDSL<UpdateModel> dsl) {
    ...
    Optional<SystemRawResource> selectOne(SelectStatementProvider selectStatement);
    ...
    List<SystemRawResource> selectMany(SelectStatementProvider selectStatement);

    @Override
    @SelectProvider(type = SqlWithRecordLockProviderAdapter.class, method = "select")
    @ResultMap("SystemResult")
    List<SystemRawResource> selectManyWithRecordLock(SelectStatementProvider selectStatement);
}
