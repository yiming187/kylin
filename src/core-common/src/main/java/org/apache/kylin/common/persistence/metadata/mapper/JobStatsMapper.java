...
import static org.apache.kylin.common.persistence.metadata.mapper.JobStatsDynamicSqlSupport.sqlTable;
...
public interface JobStatsMapper extends BasicMapper<JobStatsRawResource> {
    @Override
    default BasicColumn[] getSelectList() {
        return getSelectListWithAdditions(sqlTable.timeOfDay);
    }
    ...
    @Override
    default UpdateDSL<UpdateModel> updateAllColumns(JobStatsRawResource record, UpdateDSL<UpdateModel> dsl) {
        dsl = BasicMapper.super.updateAllColumns(record, dsl);
        return dsl.set(sqlTable.timeOfDay).equalTo(record::getTimeOfDay);
    }

    @Override
    @SelectProvider(type = SqlProviderAdapter.class, method = "select")
    @ResultMap("JobStatsResult")
    Optional<JobStatsRawResource> selectOne(SelectStatementProvider selectStatement);

    @Override
    @SelectProvider(type = SqlProviderAdapter.class, method = "select")
    @Results(id = "JobStatsResult", value = {
            ...
            @Result(column = "time_of_day", property = "timeOfDay", jdbcType = JdbcType.BIGINT),
    ...
    List<JobStatsRawResource> selectMany(SelectStatementProvider selectStatement);

    @Override
    @SelectProvider(type = SqlWithRecordLockProviderAdapter.class, method = "select")
    @ResultMap("JobStatsResult")
    List<JobStatsRawResource> selectManyWithRecordLock(SelectStatementProvider selectStatement);
}
