...
import static org.apache.kylin.common.persistence.metadata.mapper.UserInfoDynamicSqlSupport.sqlTable;
...
public interface UserInfoMapper extends BasicMapper<UserInfoRawResource> {
    @Override
    ...
    @Override
    default UpdateDSL<UpdateModel> updateAllColumns(UserInfoRawResource record, UpdateDSL<UpdateModel> dsl) {
        dsl = BasicMapper.super.updateAllColumns(record, dsl);
        return dsl.set(sqlTable.username).equalTo(record::getUsername);
    }

    @Override
    default BasicColumn[] getSelectList() {
        return getSelectListWithAdditions(sqlTable.username);
    }
    ...
    Optional<UserInfoRawResource> selectOne(SelectStatementProvider selectStatement);
    ...
    List<UserInfoRawResource> selectMany(SelectStatementProvider selectStatement);

    @Override
    @SelectProvider(type = SqlWithRecordLockProviderAdapter.class, method = "select")
    @ResultMap("UserResult")
    List<UserInfoRawResource> selectManyWithRecordLock(SelectStatementProvider selectStatement);
}
