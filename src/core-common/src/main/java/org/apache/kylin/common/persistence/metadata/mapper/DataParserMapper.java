...
import static org.apache.kylin.common.persistence.metadata.mapper.DataParserDynamicSqlSupport.sqlTable;
...
public interface DataParserMapper extends BasicMapper<DataParserRawResource> {

    @Override
    @SelectProvider(type = SqlProviderAdapter.class, method = "select")
    @Results(id = "DataParserResult", value = {
    ...
    List<DataParserRawResource> selectMany(SelectStatementProvider selectStatement);

    @Override
    @SelectProvider(type = SqlProviderAdapter.class, method = "select")
    @ResultMap("DataParserResult")
    Optional<DataParserRawResource> selectOne(SelectStatementProvider selectStatement);

    @Override
    default UpdateDSL<UpdateModel> updateAllColumns(DataParserRawResource record, UpdateDSL<UpdateModel> dsl) {
        dsl = BasicMapper.super.updateAllColumns(record, dsl);
        return dsl.set(sqlTable.jarName).equalTo(record::getJarName).set(sqlTable.className)
                .equalTo(record::getClassName);
    }
    ...
    default BasicColumn[] getSelectList() {
        return getSelectListWithAdditions(sqlTable.jarName, sqlTable.className);
    }

    @Override
    @SelectProvider(type = SqlWithRecordLockProviderAdapter.class, method = "select")
    @ResultMap("DataParserResult")
    List<DataParserRawResource> selectManyWithRecordLock(SelectStatementProvider selectStatement);
}
