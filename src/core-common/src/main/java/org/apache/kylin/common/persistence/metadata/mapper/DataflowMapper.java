...
import static org.apache.kylin.common.persistence.metadata.mapper.DataFlowDynamicSqlSupport.sqlTable;
...
public interface DataflowMapper extends BasicMapper<DataflowRawResource> {
    ...
    Optional<DataflowRawResource> selectOne(SelectStatementProvider selectStatement);

    @Override
    @SelectProvider(type = SqlProviderAdapter.class, method = "select")
    @Results(id = "DataFlowResult", value = {
    ...
    List<DataflowRawResource> selectMany(SelectStatementProvider selectStatement);

    @Override
    @SelectProvider(type = SqlWithRecordLockProviderAdapter.class, method = "select")
    @ResultMap("DataFlowResult")
    List<DataflowRawResource> selectManyWithRecordLock(SelectStatementProvider selectStatement);
}
