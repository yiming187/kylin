...
import static org.apache.kylin.common.persistence.metadata.mapper.QueryRecordDynamicSqlSupport.sqlTable;
...
public interface QueryRecordMapper extends BasicMapper<QueryRecordRawResource> {
    ...
    @Override
    @SelectProvider(type = SqlProviderAdapter.class, method = "select")
    @Results(id = "QueryRecordResult", value = {
    ...
    List<QueryRecordRawResource> selectMany(SelectStatementProvider selectStatement);

    @Override
    @SelectProvider(type = SqlWithRecordLockProviderAdapter.class, method = "select")
    @ResultMap("QueryRecordResult")
    List<QueryRecordRawResource> selectManyWithRecordLock(SelectStatementProvider selectStatement);
}
