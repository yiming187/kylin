...
import static org.apache.kylin.common.persistence.metadata.mapper.ResourceGroupDynamicSqlSupport.sqlTable;
...
public interface ResourceGroupMapper extends BasicMapper<ResourceGroupRawResource> {
    @Override
    ...
    @Override
    @SelectProvider(type = SqlProviderAdapter.class, method = "select")
    @Results(id = "ResourceGroupResult", value = {
    ...
    List<ResourceGroupRawResource> selectMany(SelectStatementProvider selectStatement);

    @Override
    @SelectProvider(type = SqlWithRecordLockProviderAdapter.class, method = "select")
    @ResultMap("ResourceGroupResult")
    List<ResourceGroupRawResource> selectManyWithRecordLock(SelectStatementProvider selectStatement);
}
