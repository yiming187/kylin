...
import static org.apache.kylin.common.persistence.metadata.mapper.HistorySourceUsageDynamicSqlSupport.sqlTable;
...
public interface HistorySourceUsageMapper extends BasicMapper<HistorySourceUsageRawResource> {
    ...
    default BasicColumn[] getSelectList() {
        return getSelectListWithAdditions(sqlTable.createTime);
    }
    ...
    @Override
    default UpdateDSL<UpdateModel> updateAllColumns(HistorySourceUsageRawResource record, UpdateDSL<UpdateModel> dsl) {
        dsl = BasicMapper.super.updateAllColumns(record, dsl);
        return dsl.set(sqlTable.createTime).equalTo(record::getCreateTime);
    }

    @Override
    @SelectProvider(type = SqlProviderAdapter.class, method = "select")
    @ResultMap("HistorySourceUsageResult")
    Optional<HistorySourceUsageRawResource> selectOne(SelectStatementProvider selectStatement);

    @Override
    @SelectProvider(type = SqlProviderAdapter.class, method = "select")
    @Results(id = "HistorySourceUsageResult", value = {
    ...
    List<HistorySourceUsageRawResource> selectMany(SelectStatementProvider selectStatement);

    @Override
    @SelectProvider(type = SqlWithRecordLockProviderAdapter.class, method = "select")
    @ResultMap("HistorySourceUsageResult")
    List<HistorySourceUsageRawResource> selectManyWithRecordLock(SelectStatementProvider selectStatement);
}
