...
import static org.apache.kylin.common.persistence.metadata.mapper.TableExdDynamicSqlSupport.sqlTable;
...
public interface TableExdMapper extends BasicMapper<TableExdRawResource> {
    @Override
    default UpdateDSL<UpdateModel> updateAllColumns(TableExdRawResource record, UpdateDSL<UpdateModel> dsl) {
        dsl = BasicMapper.super.updateAllColumns(record, dsl);
        return dsl.set(sqlTable.tableName).equalTo(record::getTableName);
    }

    @Override
    default BasicColumn[] getSelectList() {
        return getSelectListWithAdditions(sqlTable.tableName);
    }
    ...
    @Override
    @SelectProvider(type = SqlProviderAdapter.class, method = "select")
    @Results(id = "TableExdResult", value = {
    ...
    List<TableExdRawResource> selectMany(SelectStatementProvider selectStatement);

    @Override
    @SelectProvider(type = SqlWithRecordLockProviderAdapter.class, method = "select")
    @ResultMap("TableExdResult")
    List<TableExdRawResource> selectManyWithRecordLock(SelectStatementProvider selectStatement);
}
