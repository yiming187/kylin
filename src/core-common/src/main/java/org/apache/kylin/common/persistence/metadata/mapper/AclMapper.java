...
import static org.apache.kylin.common.persistence.metadata.mapper.AclDynamicSqlSupport.sqlTable;
...
public interface AclMapper extends BasicMapper<AclRawResource> {
    ...
    @ResultMap("AclResult")
    Optional<AclRawResource> selectOne(SelectStatementProvider selectStatement);

    @Override
    @SelectProvider(type = SqlProviderAdapter.class, method = "select")
    @Results(id = "AclResult", value = { @Result(column = "id", property = "id", jdbcType = JdbcType.BIGINT, id = true),
    ...
    List<AclRawResource> selectMany(SelectStatementProvider selectStatement);

    @Override
    @SelectProvider(type = SqlWithRecordLockProviderAdapter.class, method = "select")
    @ResultMap("AclResult")
    List<AclRawResource> selectManyWithRecordLock(SelectStatementProvider selectStatement);
}
