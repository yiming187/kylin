...
import static org.apache.kylin.common.persistence.metadata.mapper.IndexPlanDynamicSqlSupport.sqlTable;
...
public interface IndexPlanMapper extends BasicMapper<IndexPlanRawResource> {
    ...
    @Override
    @SelectProvider(type = SqlProviderAdapter.class, method = "select")
    @Results(id = "IndexPlanResult", value = {
    ...
    List<IndexPlanRawResource> selectMany(SelectStatementProvider selectStatement);

    @Override
    @SelectProvider(type = SqlWithRecordLockProviderAdapter.class, method = "select")
    @ResultMap("IndexPlanResult")
    List<IndexPlanRawResource> selectManyWithRecordLock(SelectStatementProvider selectStatement);
}
