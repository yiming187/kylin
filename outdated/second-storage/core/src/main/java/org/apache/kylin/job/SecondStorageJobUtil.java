...
public class SecondStorageJobUtil extends ExecutableUtil {
    ...
    private static boolean isValidateSegments(NDataSegment segment, SegmentRange<?> range) {
        return segment.getSegRange().startStartMatch(range) && segment.getSegRange().endEndMatch(range)
                && (segment.getStatus() == SegmentStatusEnum.READY || segment.getStatus() == SegmentStatusEnum.WARNING);
    }
    ...
    public void computeLayout(JobParam jobParam) {
        ...
        final NDataflow df = NDataflowManager.getInstance(config, project).getDataflow(model);

        List<NDataSegment> segments = df.getSegments().stream()
                .filter(segment -> isValidateSegments(segment, segment.getSegRange())).collect(Collectors.toList());
        ...
        Map<Long, List<LayoutEntity>> layouts = segments.stream()
                .flatMap(nDataSegment -> nDataSegment.getLayoutsMap().values().stream()).map(NDataLayout::getLayout)
                .filter(SecondStorageUtil::isBaseTableIndex).collect(Collectors.groupingBy(LayoutEntity::getIndexId));
        ...
        if (!segments.get(0).getSegRange().isInfinite()) {
            ...
            Preconditions.checkState(
                    baseTableIndex.getColumns().stream().map(TblColRef::getTableDotName)
                            .anyMatch(col -> Objects.equals(col, partitionCol)),
        ...
        for (Map.Entry<Long, List<LayoutEntity>> entry : layouts.entrySet()) {
            LayoutEntity layoutEntity = entry.getValue().stream()
                    .filter(l -> l.isBaseIndex() && IndexEntity.isTableIndex(l.getId())).findFirst().orElse(null);
